...
// The teardrop tip must always point inward — i.e. sit closer to the gauge
// center than the arc point, at distance r-d, for every angle. This is what
// the old rotate-based marker got wrong on the sides (0° / 180°).
func TestMarkerTipPointsInward(t *testing.T) {
	...
	for _, a := range []float64{0, 45, 90, 135, 180} {
		...
		if dist >= r {
			t.Errorf("a=%.0f°: tip dist %.2f not inward (>= r %.0f)", a, dist, r)
		}
